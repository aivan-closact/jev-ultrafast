(() => {
  ...
  // the smallest enclosing text that says more than the label itself tells them apart.
  const counts={}; for (const a of actions) counts[a.label]=(counts[a.label]||0)+1;
  for (const a of actions) {
    if ((counts[a.label]<2 && a.label!==a.role) || typeof a.node!=='number') continue;
    // Two rings: the nearest text that says more than the label (the value beside an Edit), and the
    // widest still-local text under 100 characters (the group heading that names whose value it is).
    let n=cache.nodes.get(a.node)?.parentElement, near='', far='';
    for (let i=0;i<8&&n&&n!==document.body;i++,n=n.parentElement) {
      const t=(n.innerText||'').replace(/\s+/g,' ').trim();
      if (t.length<=a.label.length+2) continue;
      if (t.length>100) break;
      near||=t; far=t;
    }
    if (near) a.context=near===far ? near : near+' | '+far;
  }
