...
Scrolling covers the document and one overflow container: the visible pane hiding the most content whose centre hit-tests to itself. `SCROLL_DOWN`/`SCROLL_UP` are offered while either can move, the wheel is dispatched at that pane's clipped centre (so the pane moves first and the document takes over at its end, as under a user's wheel), and the pane's identity and scroll position are part of the page key and marker. Options in a custom listbox are named by the control that opened them — the listbox's own name, the combobox owning it through `aria-controls`/`aria-owns`, or the one expanded combobox — as "Category → Design", and are listed first while the list is open; a button-shaped combobox reports its shown text as its value.
...
Browser mutations are not retried by transport recovery. Completed execution is logged before the next observation, including when that observation encounters a navigation. An interrupted native-select evaluation stops because its change event may already have fired. Typing uses a browser select-all command followed by CDP text insertion, so existing input contents are replaced. Uploads resolve the observed file input to a live object handle and call `DOM.setFileInputFiles`, which fires the page's own input/change events; hidden inputs behind styled buttons are indexed, and file-chooser dialogs are intercepted so a click on a Browse control cannot stall the tab.

The next observation waits for up to two animation frames or 50 ms after an interaction. Editable ARIA comboboxes instead wait for visible options, capped at 200 ms. This avoids paying for a prediction before autocomplete suggestions arrive. A new tab is first observed only once two marker reads 100 ms apart agree, the page shows something, and no region is `aria-busy="true"` (streamed content and hydration land after `readyState` is complete). Every action then settles: the observation returns as soon as the page's semantic marker differs from the one the decision was made on, or after `JEV_SETTLE_MS` (default 1500) without a change; a page that moved is then read only once it is still again, since a committed client-side navigation is an empty shell until its content streams in. Client-side navigations and streamed content land a few hundred milliseconds after the input; reading earlier recorded a false `page_changed: False` and made the agent repeat the click, restarting the same navigation each time. An explicit WAIT is the same read with no input, so it means "until the page moves"; network loading is never fast-forwarded in the recording.
...
The policy is generic, but two websites do not establish broad reliability. Name resolution covers common labels, ARIA references, and text; it is not the browser's full accessibility algorithm. Shadow roots, frames, canvas, drag-and-drop-only uploads, a second scrolling pane beside the one offered, pop-ups, and complex keyboard interactions can block progress. A valid action can still be wrong. Independent checks, rather than the model's DONE choice, determine whether the demonstrated task succeeded.
