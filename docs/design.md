...
Scrolling covers the document and one overflow container: the visible pane hiding the most content whose centre hit-tests to itself. `SCROLL_DOWN`/`SCROLL_UP` are offered while either can move, the wheel is dispatched at that pane's clipped centre (so the pane moves first and the document takes over at its end, as under a user's wheel), and the pane's identity and scroll position are part of the page key and marker.
